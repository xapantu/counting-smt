...
(assert (or (not (and (>= |s0.round| 0) (select |s0.good_p| |s0.source|))) (= (select |s0.cx| |s0.source|) |s0.val|)))
...
(check-sat)
(pop 1)
(assert (or (not (and (>= |s1.round| 0) (select |s1.good_p| |s1.source|))) (= (select |s1.cx| |s1.source|) |s1.val|)))
...
(check-sat)
(pop 1)
(assert (or (not (and (>= |s2.round| 0) (select |s2.good_p| |s2.source|))) (= (select |s2.cx| |s2.source|) |s2.val|)))
...
(push 1)
(assert (not (or (not (and (>= |s3.round| 0) (select |s3.good_p| |s3.source|))) (= (select |s3.cx| |s3.source|) |s3.val|))))
(check-sat)
(pop 1)
